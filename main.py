...
async def handle_step(message: types.Message, state: FSMContext):
    ...
    await state.update_data(step=step_num, position_index=0, interrupted=False)
...
async def handle_controls(message: types.Message, state: FSMContext):
    ...
    if message.text == "⏭️ Пропустить":
        await state.update_data(interrupted=True, position_index=position_index + 1)
        await run_positions(message.chat.id, step_num, state)

    elif message.text == "⛔ Завершить":
        await state.update_data(interrupted=True)
        await state.finish()
