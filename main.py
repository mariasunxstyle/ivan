...
Если был перерыв — вернись на 2 шага назад.""Шаг завершён. Выбирай следующий или отдохни ☀️\nЕсли был перерыв — вернись на 2 шага назад."
...
async def go_back(message: types.Message):
    ...
    state = user_state.get(user_id)
    if not state:
        user_state[user_id] = {"step": 1, "position": 0}
    else:
        if state['step'] <= 2:
            state['step'] = 1
        else:
            state['step'] -= 2
        state['position'] = 0
    await bot.send_message(user_id, f"Шаг {user_state[user_id]['step']}")
    await start_position(user_id)
